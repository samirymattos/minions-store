window.SimpleSlide = class {
  constructor(t) {
    (this.config = {
      ...
      pauseOnHover: !!t.pauseOnHover && t.pauseOnHover
    }),
      (this.pause = !1),
      (this.activeClass = "active"),
      (this.slide = document.querySelector(
        `[data-slide="${this.config.slide}"]`
      )),
  ...
  activateNav(t) {
    if (this.config.nav) {
      const i = this.items.indexOf(t),
        e = [...this.nav.children];
      e.forEach(t => t.classList.remove(this.activeClass)),
        e[i].classList.add(this.activeClass);
    }
  }
  pauseOnHover() {
    this.items.forEach(t => {
      t.addEventListener("mouseenter", () => (this.pause = !0)),
        t.addEventListener("mouseleave", () => (this.pause = !1));
    });
  }
  rotateSlide() {
    if (!this.pause) {
  ...
  createNavigation() {
    (this.nav = document.createElement("div")),
      ...
      this.items.forEach((t, i) => {
        this.nav.innerHTML += `<button data-slide-item="${i}">${i +
          1}</button>`;
      }),
  ...
  init() {
    this.bindFunctions(),
      this.config.auto && this.initAutoSlide(),
      this.config.nav && this.initNavigation(),
